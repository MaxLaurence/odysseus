...
def _list_emails(folder="INBOX", max_results=20, unresponded_only=False,
    ...
    if unread_only or unresponded_only:
        if unread_only and unresponded_only:
            criteria = "(UNSEEN UNANSWERED)"
        elif unread_only:
            criteria = "(UNSEEN)"
        else:
            # unresponded_only without unread_only: must search UNANSWERED, not
            # fall through to UNSEEN/ALL (which returned answered mail too,
            # despite the documented "emails without replies" behaviour).
            criteria = "(UNANSWERED)"
