...
def build_parser() -> argparse.ArgumentParser:
    ...
    # bead — repo-scoped, dependency-aware issue backlog (the source of truth for work)
    bead = sub.add_parser("bead", help="Beads issue backlog (bd) for a space").add_subparsers(dest="action", required=True)
    bp = bead.add_parser("list", help="List issues in a space's backlog")
    bp.add_argument("--space", dest="space_id", required=True)
    bp.add_argument("--include-closed", dest="include_closed", action="store_true")
    bp.add_argument("--limit", type=int)
    bp.set_defaults(_method="bead.list",
                    _collect=lambda a: {**_params(a, "space_id", "limit"),
                                        **({"include_closed": True} if a.include_closed else {})})
    bp = bead.add_parser("ready", help="Show unblocked, actionable work")
    bp.add_argument("--space", dest="space_id", required=True)
    bp.add_argument("--limit", type=int)
    bp.set_defaults(_method="bead.ready", _collect=lambda a: _params(a, "space_id", "limit"))
    bp = bead.add_parser("show", help="Show one issue + its dependencies")
    bp.add_argument("issue_id")
    bp.add_argument("--space", dest="space_id", required=True)
    bp.set_defaults(_method="bead.show", _collect=lambda a: _params(a, "space_id", "issue_id"))
    bp = bead.add_parser("status", help="Backlog counts (open/ready/blocked/...)")
    bp.add_argument("--space", dest="space_id", required=True)
    bp.set_defaults(_method="bead.status", _collect=lambda a: _params(a, "space_id"))
    bp = bead.add_parser("graph", help="Dependency DAG (nodes + edges)")
    bp.add_argument("--space", dest="space_id", required=True)
    bp.set_defaults(_method="bead.graph", _collect=lambda a: _params(a, "space_id"))
    bp = bead.add_parser("create", help="Create an issue (record discovered work)")
    bp.add_argument("--space", dest="space_id", required=True)
    bp.add_argument("--title", required=True)
    bp.add_argument("--type", dest="issue_type")
    bp.add_argument("--priority", type=int)
    bp.add_argument("--description", dest="description")
    bp.add_argument("--discovered-from", dest="discovered_from")
    bp.add_argument("--thread", dest="thread_id")
    bp.set_defaults(_method="bead.create",
                    _collect=lambda a: _params(a, "space_id", "title", "issue_type", "priority",
                                                "description", "discovered_from", "thread_id"))
    bp = bead.add_parser("update", help="Update an issue's status/priority/title")
    bp.add_argument("issue_id")
    bp.add_argument("--space", dest="space_id", required=True)
    bp.add_argument("--status", help="open|in_progress|blocked|closed|deferred")
    bp.add_argument("--priority", type=int)
    bp.add_argument("--title")
    bp.add_argument("--thread", dest="thread_id")
    bp.set_defaults(_method="bead.update",
                    _collect=lambda a: _params(a, "space_id", "issue_id", "status", "priority", "title", "thread_id"))
    bp = bead.add_parser("close", help="Close one or more issues")
    bp.add_argument("issue_id")
    bp.add_argument("--space", dest="space_id", required=True)
    bp.add_argument("--reason")
    bp.add_argument("--thread", dest="thread_id")
    bp.set_defaults(_method="bead.close", _collect=lambda a: _params(a, "space_id", "issue_id", "reason", "thread_id"))
    bp = bead.add_parser("dep", help="Add a dependency: <blocked> depends on <blocker>")
    bp.add_argument("--space", dest="space_id", required=True)
    bp.add_argument("--blocked", dest="blocked_id", required=True)
    bp.add_argument("--blocker", dest="blocker_id", required=True)
    bp.add_argument("--type", default="blocks")
    bp.add_argument("--thread", dest="thread_id")
    bp.set_defaults(_method="bead.dep", _collect=lambda a: _params(a, "space_id", "blocked_id", "blocker_id", "type", "thread_id"))
    bp = bead.add_parser("undep", help="Remove a dependency: <blocked> no longer depends on <blocker>")
    bp.add_argument("--space", dest="space_id", required=True)
    bp.add_argument("--blocked", dest="blocked_id", required=True)
    bp.add_argument("--blocker", dest="blocker_id", required=True)
    bp.add_argument("--thread", dest="thread_id")
    bp.set_defaults(_method="bead.dep_remove", _collect=lambda a: _params(a, "space_id", "blocked_id", "blocker_id", "thread_id"))
