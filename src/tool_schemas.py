        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "effort": {"type": "string", "enum": ["minimal", "low", "medium", "high", "max"], "description": "Reasoning effort for create_thread/update_thread/run_thread. Codex maps it to model_reasoning_effort; Claude to a thinking-token budget; other harnesses ignore it. Omit to inherit project/global default."},
                    "auth_mode": {"type": "string", "enum": ["none", "endpoint", "subscription"], "description": "How the agent CLI authenticates: 'subscription' = the owner's managed Claude/Codex OAuth login; 'endpoint' = a ModelEndpoint api-key; 'none' = the CLI's own native store. For create_thread/update_thread/run_thread."},
                    "provider": {"type": "string", "description": "Convenience provider selector that sets auth_mode + endpoint together: 'subscription:claude', 'subscription:codex', or 'endpoint:<model_endpoint_id>'. Overrides model_endpoint_id/auth_mode when given."},
